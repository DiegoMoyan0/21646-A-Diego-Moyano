    ...
    if(option === "new") {
        ...
        fetch('http://localhost:3000/api/foro', {
           ...
           if (res.ok) { 
            ...
            document.getElementById('notificationMessage').textContent = 'Posteo creado correctamente';
            setTimeout(() => {
                myModal.hide();
                location.reload();
            }, 2000);
    ...
    if(option === "edit") {
       const newPost = {
        ...
        fetch(`http://localhost:3000/api/foro/${idForm}`, {
           ...
           if(res.ok){
            notificationModal.show(); 
            document.getElementById('notificationMessage').textContent = 'Posteo editado correctamente';
            setTimeout(() => {
                myModal.hide();
                location.reload();
            }, 2000);
